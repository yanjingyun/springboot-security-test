...
import com.yjy.security.verifyCode.filter.VerifyCodeFilter;
...
//		auth.inMemoryAuthentication().withUser("user1").password(encode).roles("USER");
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.addFilterBefore(verifyCodeFilter, UsernamePasswordAuthenticationFilter.class) //添加验证码过滤器
			.authorizeRequests() // 开启请求权限配置
			.antMatchers("/authentication/*", "/login", "/verify/code/*").permitAll() // 不需要登录就可以访问
